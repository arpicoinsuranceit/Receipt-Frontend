...
@Component({
  selector: 'app-alert',
  templateUrl: './details-of-policies-report.component.html',
  styleUrls: ['./details-of-policies-report.component.css']
})
export class DetailsOfPoliciesReportComponent implements OnInit {
  // modalTitle: string;
  // modelMessage : string [];
  //modelType : string;
  choise: string = "no";
  ...
  get PickAgentCode() {
    return this.detOfPolForm.get("ic");
  }

  constructor(private dialogRef: MatDialogRef<DetailsOfPoliciesReportComponent>,@Inject(MAT_DIALOG_DATA) public data: any,
              private reportService:ReportService,private courierService:CourierDocumentService) {
    
    this.detOfPolForm.get("sp").setValue("0");
    this.detOfPolForm.get("unl").setValue("ALL");
    this.loadBranches();
  ...
  loadAgents(){
    ...
    this.reportService.getAllAgentRelatedToBranches(branches).subscribe(response => {
      ...
      this.detOfPolForm.get("ic").setValue("ALL");
    },error=>{
      alert("Error occour at Loading Branches");
    });
  ...
  yesConfirmation(){
    if(this.detOfPolForm.valid){
      this.loading_report=true;
      this.invalidForm=false;

      this.reportService.detailsOfPolicices(this.detOfPolForm.get("fromDate").value,this.detOfPolForm.get("toDate").value,this.detOfPolForm.get("ic").value
        ,this.detOfPolForm.get("unl").value,this.detOfPolForm.get("branch").value,this.detOfPolForm.get("sp").value).subscribe(response =>{
